...
static int read_ddl_log_header(const char *file_name)
{
  ...
  if (memcmp(header, ddl_log_file_magic, 4))
  {
    ...
    sql_print_warning("Wrong header in %s.  Assuming it is an old recovery "
                      "file from MariaDB 10.5 or earlier. Skipping DDL "
                      "recovery", file_name);
    goto err;
  }
...
/*
  ...
  Rename is done according to: db.name -> from_db.from_name
...
/*
  Rename a table to final table in case of ALTER TABLE ... RENAME

  Rename is done according to: db.name -> db.extra_name
  (In case of change of database, the temporary table is already
  in the final database directory).
*/

static void execute_rename_table_final(DDL_LOG_ENTRY *ddl_log_entry,
                                       handler *file,
                                       char *from_path,
                                       char *to_path)
{
  LEX_CSTRING tmp_from_db=   ddl_log_entry->from_db;
  LEX_CSTRING tmp_from_name= ddl_log_entry->from_name;

  ddl_log_entry->from_db=   ddl_log_entry->db;
  ddl_log_entry->from_name= ddl_log_entry->extra_name;
  /* Rename db.name -> from_db.from_name */
  execute_rename_table(ddl_log_entry, file, from_path, to_path);
  ddl_log_entry->from_db=   tmp_from_db;
  ddl_log_entry->from_name= tmp_from_name;
}
    ...
    case DDL_ALTER_TABLE_PHASE_INPLACE:
    {
      ...
      */
      ...
      if (!access(from_path, F_OK))             // Does #sql-alter.. exists?
      {
        ...
        */
        from_path[length - reg_ext_length]= 0;  // Remove .frm
        if (!hton->check_version ||
            (!hton->check_version(hton, from_path, &version,
                                  ddl_log_entry->unique_id)))
        {
          ...
          */
          ...
          from_path[length - reg_ext_length]= FN_EXTCHAR;
      ...
      if (is_renamed && new_version_ready)
      {
        ...
        execute_rename_table_final(ddl_log_entry, file, from_path, to_path);
      }
...
int ddl_log_execute_recovery()
{
  ...
  */
  ...
  if (create_ddl_log())
    DBUG_RETURN(1);
  if (count > 0)
    sql_print_information("Crash DDL recovery executed %u entries.", count);
  DBUG_RETURN(error);
}
